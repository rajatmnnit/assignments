...
To acurately target photographers, we need to classify the customers buying this bundle into “true” photographers v/s “non” photographers. If we are able to do that, the business will be able to,
...
```
Being able to identify “true” photographers will help Adobe marketing team by

* enabling them to target marketing communication to the correct set of users. For example, sending a tutorial link on "how to organize your photos in Lightroom" to a UX/UI designer who purchased Photography plan to get access to Photoshop for his/her work, may be a waste of Adobe's marketing dollars and users' screen time.
* enabling them to measure accurately and increase users' engagement with products and eventually steer the “non” photographers to the plans which may be more suitable for them. Thus increasing retention and eventually ARR in longer term.
&nbsp;
...
```
We will build models based on various member attributes and their usage of the produts and product features, to segment Paid Photography segment users into “true” Photographers and “non” Photographers segments. The output of this project will be a table consisting of all qualifying members with their corresponding segment at a given snapshot date/time.
...
* Members' profile consisting various attributes like geo location, plan purchased etc. This data resides in Adobe's financial systems and is synched over to a centralized Hadoop cluster.
* Members' product and product feature usage data collected by various Adobe apps running on users' machines.
* Results from a survey where users have identified themselves into various job functions.
...
### b. Machine Resources
No special hardware is required at this time based on the estimated volume. For future iterations, specialized hardware or data processing platforms may be required depending on the growth in size of data and desired frequency of model update.
&nbsp;
...
```
The first version of the project will take 7 weeks to complete.
...
### a. **Ingest**:
Data exploration, clean-up and ingestion will take three weeks and is estimated to be completed by 05/02/2018.

### b. **Model**:
Model(s) will be built on the ingested data and will take three weeks. The will be available for validation by 05/23/2018.

### c. **Validate**:
No validation testing on production data planned yet.
